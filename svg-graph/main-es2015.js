...
class Quadrangle {
    ...
    static getCentroid(nodes) {
        const centerX = nodes.map(node => node.posX).reduce((sum, current) => sum + current) / nodes.length;
        const centerY = nodes.map(node => node.posY).reduce((sum, current) => sum + current) / nodes.length;
...
class Mesh {
    ...
    static subdivideEdge(edge) {
        const centerX = (edge.nodeA.posX + edge.nodeB.posX) / 2;
        const centerY = (edge.nodeA.posY + edge.nodeB.posY) / 2;
        const centroid = new _primitive_node__WEBPACK_IMPORTED_MODULE_2__["Node"](centerX, centerY);
        const edgeA = new _primitive_edge__WEBPACK_IMPORTED_MODULE_1__["Edge"](edge.nodeA, centroid);
        const edgeB = new _primitive_edge__WEBPACK_IMPORTED_MODULE_1__["Edge"](centroid, edge.nodeB);
        return { centroid, edgeA, edgeB };
    }
    subdivide() {
        ...
        this.edges.forEach(edge => faceMap.set(edge, []));
        this.faces.forEach(face => {
            edgeMap.set(face, []);
            nodeMap.set(face, [...face.nodes, face.centroid]);
            centroidMap.set(face, face.centroid);
            face.edges.forEach(edge => faceMap.get(edge).push(face));
        });
        const nodes = this.nodes.concat(this.faces.map(face => face.centroid));
        const edges = [];
        this.edges.forEach(edge => {
            const affectedFaces = faceMap.get(edge);
            const subdivided = Mesh.subdivideEdge(edge);
            ...
            affectedFaces.forEach(face => {
                ...
                const faceCentroid = centroidMap.get(face);
                const centroidEdge = new _primitive_edge__WEBPACK_IMPORTED_MODULE_1__["Edge"](subdivided.centroid, faceCentroid);
        ...
        const faces = this.faces.map(face => {
            const faceNodes = face.nodes.map(node => edgeMap.get(face).filter(edge => edge.containsNode(node))
                .map(edge => edge.getOtherEnd(node))
...
class Scene {
    constructor(meshes) {
        const nodeA = new _primitive_node__WEBPACK_IMPORTED_MODULE_1__["Node"](10, 10);
        const nodeB = new _primitive_node__WEBPACK_IMPORTED_MODULE_1__["Node"](990, 10);
        const nodeC = new _primitive_node__WEBPACK_IMPORTED_MODULE_1__["Node"](990, 990);
        const nodeD = new _primitive_node__WEBPACK_IMPORTED_MODULE_1__["Node"](10, 990);
        const points = [nodeA, nodeB, nodeC, nodeD];
        const edges = [
        ...
        const face = [new _face_quadrangle__WEBPACK_IMPORTED_MODULE_3__["Quadrangle"](points, edges)];
        if (!meshes) {
    ...
    subdivideMeshes() {
        return new Scene(this.meshes.map(mesh => mesh.subdivide()));
    }
}
...
let GraphComponent = GraphComponent_1 = class GraphComponent {
    constructor(settings) {
        ...
        this.currentScene = settings.getSubdivisionObservable()
            .pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["map"])(subdivisions => GraphComponent_1.subdivideScene(this.baseScene, subdivisions)));
    }
    static subdivideScene(scene, divisions) {
        if (divisions <= 0) {
            return scene;
        }
...
let SettingsService = class SettingsService {
    constructor() {
        ...
        this.subdivisions.asObservable().subscribe(value => console.log('value:' + value));
    }
    setSubdivisions(subdivisions) {
        this.subdivisions.next(subdivisions);
    }
    getSubdivisionObservable() {
...
let RectComponent = class RectComponent {
    constructor() {
    }
    ngOnInit() {
    }
...
let SvgelementsModule = class SvgelementsModule {
};
